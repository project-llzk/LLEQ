...
public:
  ...
  template <typename AnchorT> void eraseState(AnchorT anchor) {
...
protected:
  ...
  virtual void onUpdate(DataFlowSolver *solver) const {
    for (const DataFlowSolver::WorkItem &item : dependents) {
      llvm::dbgs() << "\t* program point " << item.first << "\n";
      solver->enqueue(item);
    }
  }
...
protected:
  ...
  template <typename AnchorT> void registerAnchorKind() {
...
template <> struct DenseMapInfo<mlir::ProgramPoint> {
